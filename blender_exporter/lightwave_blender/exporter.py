...
def export_objects(registry: SceneRegistry):
    ...
    for inst in registry.depsgraph.object_instances:
        ...
        if objType in {'MESH', 'CURVE', 'SURFACE', 'META', 'FONT', 'CURVES'}:
            nodes = registry.export(object_eval.original.data,
            ...
            for (mat_id, node) in enumerate(nodes):
                if node.name == "instance": # this case happens when we do our hair to feather conversion
                    result.append(node)
                else: # default case
                    result.append(export_entity(registry, inst, node, mat_id))
