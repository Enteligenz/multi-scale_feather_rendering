...
class ExportLightwave(bpy.types.Operator, ExportHelper):
    ...
    generate_feathers: BoolProperty(
        name="Generate feathers based on curves in scene",
        description="Enable this to generate feathers based on curves in the scene instead of exporting the curves. NOTE: This will cause other settings in the Curves section to be ignored.",
        default=False,
    )

    export_curves_as_feathers: BoolProperty(
        name="Export curves in feather format",
        description="Disable this to export curves as curves objects instead (only difference is in xml-file, can be changed anytime there)",
        default=True,
    )
...
class LIGHTWAVE_PT_export_include(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        layout.separator()
        col = layout.column(heading="Curves")
        col.prop(operator, 'generate_feathers', text="Generate Feathers")
        col.prop(operator, 'export_curves_as_feathers', text="Export as Feathers")
