...
def generate_feather(registry: SceneRegistry,
                     id_suffix: str = "", # optional suffix that gets added at the end of the file names
                     num_feathers: int = 1,
                     spine_dir: Vec3 = None, 
    ...
    Returns a tuple with the relative file paths to the 3 generated files, in order of:
    spines, barbs, barbules.
    """
    # TODO IDEAS:
    ...
    shape_factors = {
        ...
        "broad": {"spine_curvature": 0.3, "barb_angle_blend": 0.3, "barb_curve_strength": 0.4},
        "narrow": {"spine_curvature": 0.1, "barb_angle_blend": 0.7, "barb_curve_strength": 0.4}
    } # TODO narrow type has some overlapping/negative angle bug
    ...
    # 5. write to .ply file
    rel_path_spines, abs_path_spines = create_path(registry, "feather_spines" + id_suffix)
    rel_path_barbs, abs_path_barbs = create_path(registry, "feather_barbs" + id_suffix)
    rel_path_barbules, abs_path_barbules = create_path(registry, "feather_barbules" + id_suffix)

    write_ply_file(abs_path_spines, segments_spines, len(all_spines))
    write_ply_file(abs_path_barbs, segments_barbs, len(all_barbs))
    write_ply_file(abs_path_barbules, segments_barbules, len(all_barbules))
    ...
    print(f"Written to: {abs_path_spines}, {abs_path_barbs}, {abs_path_barbules}")

    return (rel_path_spines, rel_path_barbs, rel_path_barbules)