...
public:
    ...
    void finish() {
        ...
        // Also append to a results file
        if (m_name == "render") {
            std::ofstream outfile("results.txt", std::ios::app);
            if (outfile.is_open()) {
                // Check if file already has content, if so prepend " + "
                std::ifstream check("results.txt");
                bool isEmpty = check.peek() == std::ifstream::traits_type::eof();
                check.close();

                if (isEmpty)
                    outfile << tfm::format("%.2f", m_timer.getElapsedTime());
                else
                    outfile << tfm::format(" + %.2f", m_timer.getElapsedTime());
            }
        }
