...
typedef double Float; // Do not forget to also change Epsilon!
...
/// @brief Pi / 2
static constexpr Float Pi2 = Float(1.57079632679489661923);
/// @brief Pi / 4
static constexpr Float Pi4 = Float(0.78539816339744830961);
/// @brief sqrt(2)
static constexpr Float Sqrt2 = Float(1.41421356237309504880);

/// @brief Multiply by this constant to convert degrees into radians.
static constexpr Float Deg2Rad = Pi / Float(180.0);
/// @brief Multiply by this constant to convert radians into degrees.
static constexpr Float Rad2Deg = Float(180.0) * InvPi;
 ...
 */
static constexpr Float Epsilon = Float(1e-8); // TODO see which values works best, might even be down to 1e-12! // 1e-4 for float, 1e-8 for double
...
/**
 * @brief Smoothstep functon, which is a sigmoid-like interpolation.
 * Code is taken from: https://en.wikipedia.org/wiki/Smoothstep
 */
inline Float smoothstep (Float edge0, Float edge1, Float x) {
    // Scale, and clamp to 0..1 range
    x = clamp((x - edge0) / (edge1 - edge0), Float(0.0f), Float(1.0f));

    return x * x * (Float(3.0f) - Float(2.0f) * x);
}
...
public:
    ...
    friend auto clamp(const TVector &a, const TVector &min, const TVector &max) { TVector BUILD1(clamp(a[i], min[i], max[i])); }
    friend auto sqrt(const TVector &a) { TVector BUILD1(sqrt(a[i])); }
    friend auto sqr(const TVector &a) { TVector BUILD1(sqr(a[i])); }
    friend auto cos(const TVector &a) { TVector BUILD1(cos(a[i])); }
    friend auto exp(const TVector &a) { TVector BUILD1(exp(a[i])); }
};
...
public:
    ...
    static auto identity() { TMatrix BUILD2(row == column ? Float(1.0) : Float(0.0)) }
};
...
/// @brief A four-dimensional point with integer components (used for 4D images).
using Point4i = TPoint<int, 4>;
/// @brief A four-dimensional point with Floating point components (used for 4D images).
using Point4 = TPoint<Float, 4>;
...
/// @brief A four-dimensional vector with integer components (used for 4D images).
using Vector4i = TVector<int, 4>;

/// @brief A three-dimensional point with floating point components. Used for parser to bridge float-double.
using Point3f = TPoint<float, 3>;
/// @brief A three-dimensional vector with floating point components. Used for parser to bridge float-double.
...
/// @brief An integer 4D box (e.g., to describe the blocks of a 4D image).
using Bounds4i = TBounds<int, 4>;
...
struct Intersection : public SurfaceEvent {
    ...
    bool testAlpha(const Point2 &uv, Sampler &rng) const;
    ...
    /// isLow is true if a low detail material should be used, false otherwise.
    BsdfSample sampleBsdf(Sampler &rng, bool isLow) const;
    BsdfEval evaluateBsdf(const Vector &wi, bool isLow) const;
    BsdfEval evaluateBsdf(const Vector &wi, const ref<Bsdf> bsdf) const;

    Light *light() const;
};

/// @brief Extra information that is relevant for LOD and should be passed along as a parameter in intersect() methods.
struct Payload {
    /// @brief Current bounce depth. On the first bounce we have depth=1.
    int depth = 1;
    /// @brief The pdf result from the last surface bounce we have done.
    Float pdf = Float(1.0);
    /// @brief The distance the ray has traveled so far before the current bounce.
    Float traveledDistance = Float(0.0);

    /// @brief The light PDF, not normalized over number of lights in the scene.
    Float lightPDF = 1;
    /// @brief Whether some previous material we bounced off of was rough.
    bool wasRough = false;
    /// @brief Distance the ray has traveled. If we bounce from a rough material at any point, is set to infinity.
    Float totalDistance = 0;
    /// @brief Pointer to the camera of the scene.
    Camera* camera = nullptr;
    /// @brief Whether we are currently doing an intersection for NEE.
    bool isForNEE = false;
    /// @brief The footprint of the last bounce.
    Float lastFootprint = 0;

    /// @brief A flag that is true if the surface touched in the previous intersection was a high detail one.
    bool isComingFromHD = true;
    /// @brief A flag that is true if the last touched surface was a high detail one.
    bool currentComingFromHD = true;
    /// @brief The ID of the feather touched in the previous intersection (if any, otherwise -1).
    int32_t lastFeatherID = -1;
    /// @brief The ID of the feather that was last touched in the ongoing intersection.
    int32_t currentFeatherID = -2;
    /// @brief The (hashed) ID of the last feathers object that was touched in the previous intersection.
    int32_t lastObjID = -1;
    /// @brief The (hashed) ID of the last touched feathers object in the ongoing intersection.
    int32_t currentObjID = -2;
