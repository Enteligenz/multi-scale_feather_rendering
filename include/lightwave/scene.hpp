...
#include <vector>
#include <lightwave/math.hpp> // This import is only done for Float.
...
public:
    ...
    Intersection intersect(const Ray &ray, Payload &p, Sampler &rng) const;
    /// @brief Reports whether any intersection up to a given maximal distance
    /// exists (used for testing visibility of light sources).
    bool intersect(const Ray &ray, Payload &p, Float tMax, Sampler &rng) const;
