...
public:
     ...
     * be dismissed.
     */
    virtual bool intersect(const Ray &ray, Intersection &its, Payload &p,
                           Sampler &rng) const = 0;
    ...
    /// 
    /**
     * @brief Passes relevant information like material downwards so that it can be used by feather generation.
     * This should be implemented by Feathers and all shapes that are acceleration structures.
     * All info that should be passed downward should be given in the parameters.
     */
    virtual void passInfoToFeathers(ref<Bsdf> bsdf) {}
};
