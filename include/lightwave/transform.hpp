...
public:
    ...
    // Taken from ART.
    Transform(const Matrix3x3 &matrix) { // Assumes orthonormality
        ...
        Matrix3x3 inv = matrix.transpose();
    ...
    /// @brief Appends a rotation to a new basis at the origin (0,0,0) to this transform.
    /// It is assumed that the original basis are the default axes of the coordinate system. 
    /// spanU and spanV are two vectors that define the new basis.
    void rotateToPlane(const Vector &spanU, const Vector &spanV) {
        const Vector u = spanU.normalized();
        const Vector v = spanV.normalized();
        const Vector w = (u.cross(v)).normalized();

        Matrix4x4 rotationMatrix = Matrix4x4::identity();
        for (int32_t i = 0; i < 3; i++) {
            rotationMatrix(i, 0) = u[i];
            rotationMatrix(i, 1) = v[i];
            rotationMatrix(i, 2) = w[i];
        }

        m_transform = rotationMatrix * m_transform;
        m_inverse   = m_inverse * rotationMatrix.transpose();
    }
