...
BsdfSample Intersection::sampleBsdf(Sampler &rng, bool isLow) const {
    PROFILE("Sample Bsdf")
    Bsdf *targetBsdf;
    if (isLow && instance->lowBsdf()) targetBsdf = instance->lowBsdf();
    else targetBsdf = instance->bsdf();

    if (!instance || !targetBsdf)
        return BsdfSample::invalid();
    assert_normalized(wo, {});
    auto bsdfSample = targetBsdf->sample(uv, shadingFrame().toLocal(wo), rng);
    if (bsdfSample.isInvalid())
        return bsdfSample;
    assert_normalized(bsdfSample.wi, {
        logger(EError, "offending BSDF: %s", targetBsdf->toString());
        logger(EError, "  input was: %s with length %f", wo, wo.length());
    });
    bsdfSample.wi = shadingFrame().toWorld(bsdfSample.wi);
    // assert_normalized(bsdfSample.wi, {}); // Commented out because the tangent is NaN sometimes and idk why, so this is just annoying
    if (!check_normalized(bsdfSample.wi)) {
        logger(EWarn, "bsdfSample.wi is broken (probably due to the tangent), thus this sample will be rejected.");
        bsdfSample = BsdfSample::invalid();
    }
    return bsdfSample;
}

BsdfEval Intersection::evaluateBsdf(const Vector &wi, bool isLow) const {
    ...
    return targetBsdf->evaluate(uv, shadingFrame().toLocal(wo), shadingFrame().toLocal(wi));
}

BsdfEval Intersection::evaluateBsdf(const Vector &wi, ref<Bsdf> bsdf) const {
    PROFILE("Evaluate Bsdf")
    
    if (!instance || !bsdf)
        return BsdfEval::invalid();
    return bsdf->evaluate(uv, shadingFrame().toLocal(wo), shadingFrame().toLocal(wi));
}

bool Intersection::testAlpha(const Point2 &testUV, Sampler &rng) const {
    if (!alphaMask) return true;
    return alphaMask->scalar(testUV) > rng.next();
}

Light *Intersection::light() const {
    if (!instance) return background;
