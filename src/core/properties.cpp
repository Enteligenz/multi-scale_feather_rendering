...
template <> int parse_string(const std::string &str) { return std::stoi(str); }
...
template <typename T> T parse_vector_string(const std::string &str) {
    ...
    for (int dim = 0; dim < result.Dimension; dim++) {
        if (i >= str.size()) {
            if (dim == 1) {
                // if only a single value is specified, set all components to
                // that value
                return T(result[0]);
            }
...
template <> PlaneFindingMode parse_string<PlaneFindingMode>(const std::string& str) {
    if (str == "standard") return PlaneFindingMode::Standard;
    if (str == "pca") return PlaneFindingMode::PCA;
    if (str == "svd") return PlaneFindingMode::SVD;
    lightwave_throw("invalid FeatherMode: %s", str.c_str());
}
