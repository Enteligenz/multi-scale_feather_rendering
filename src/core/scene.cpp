...
Intersection Scene::intersect(const Ray &ray, Payload &p, Sampler &rng) const {
    PROFILE("Intersect")

    Intersection its(-ray.direction);
    m_shape->intersect(ray, its, p, rng);
    if (!its) {
...
bool Scene::intersect(const Ray &ray, Payload &p, Float tMax, Sampler &rng) const {
    PROFILE("Shadow ray")

    Intersection its(-ray.direction, tMax * (1 - Epsilon));
    return m_shape->intersect(ray, its, p, rng);
}
